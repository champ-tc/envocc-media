...
/* ---------- helper: ดึง token จาก session ในหลายรูปแบบ ---------- */
type SessionLike =
    | {
        token?: string;
        accessToken?: string;
        user?: { token?: string; accessToken?: string } | null;
    }
    | null
    | undefined;

function getAuthToken(session: SessionLike): string | undefined {
    return (
        session?.token ||
        session?.accessToken ||
        session?.user?.token ||
...
export default function PersonalPage() {
    ...
    const authToken = getAuthToken(session as SessionLike);
    ...
    useEffect(() => {
        if (session) {
            fetchUserData(`/api/users/${idParam}`, setUserData, "ไม่สามารถดึงข้อมูลส่วนตัวได้");
        }
    }, [session, idParam, fetchUserData]);
