        ...
        const fetchBorrows = async () => {
            try {
                const res = await fetch(
                    `/api/borrow?page=${currentPage}&limit=${itemsPerPage}`, // ✅ เรียก endpoint ที่ถูก
                    ...
                    <div className="bg-white rounded-lg shadow-lg max-w-6xl w-full p-8 mt-4 lg:ml-52">
                        ...
                        <div className="grid grid-cols-1 gap-8 mt-6 sm:grid-cols-2 lg:grid-cols-4">
                            {currentBorrows.map((item) => {
                                // ✅ ใช้ image_url เป็นหลัก ถ้าไม่มีค่อย fallback ไปที่ /borrows/<filename>
                                const fallback = item.borrow_images
                                    ? `/borrows/${item.borrow_images}`
                                    : "";
                                    ...
                                    >
                                        {imgSrc ? (
                                            <Image
                                                ...
                                                height={240}
                                                priority
                                                // ถ้าเป็น URL ภายนอกและยังไม่ได้ whitelist ใน next.config ให้ใส่ unoptimized
